...
func (r *repository) GetJournalsByAuthor(ctx context.Context, author string) ([]*model.Journal, error) {
	// Mysql query
	query := "SELECT j.journal_id, j.archieve_id, j.title, j.authors, j.abstract, j.link, j.pdf_link, j.published, j.created_at, j.updated_at FROM journal AS j WHERE j.authors LIKE ?"
